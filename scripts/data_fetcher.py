...
class DataFetcher:
    ...
    def fetch_for_github(self):
        """专门用于GitHub Actions的数据获取方法，返回JSON格式数据而不推送到HA"""
        from datetime import datetime
        ...
        driver.maximize_window()
        time.sleep(self.RETRY_WAIT_TIME_OFFSET_UNIT)
        logging.info("Webdriver initialized for GitHub Actions mode.")
            ...
            try:
                ...
                if current_userid in self.IGNORE_USER_ID:
                    ...
                    user_data = {
                        ...
                        "last_daily_usage": float(last_daily_usage) if last_daily_usage is not None else None,
                        "yearly_charge": float(yearly_charge) if yearly_charge is not None else None,
                        "yearly_usage": float(yearly_usage) if yearly_usage is not None else None,
                        "month_charge": float(month_charge) if month_charge is not None else None,
                        "month_usage": float(month_usage) if month_usage is not None else None,
                    }
                    
                    result_data["users"].append(user_data)
                    logging.info(f"User {user_id} data collected successfully!")
                    
                    time.sleep(self.RETRY_WAIT_TIME_OFFSET_UNIT)
            except Exception as e:
                logging.error(f"The user {user_id} data fetching failed: {e}")
                continue
